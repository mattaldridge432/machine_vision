...
MIN_HAND_TO_LITTER_DISTANCE_INCREASE = 50  # pixels
MIN_CAR_TO_LITTER_DISTANCE_INCREASE = 100  # pixels
FACE_PROXIMITY_THRESHOLD = 300  # Max distance for face-litter association
PLATE_PROXIMITY_THRESHOLD = 400  # Max distance for plate-litter association
...
# NEW: Bounding box export settings
EXPORT_WITH_BOUNDING_BOXES = True  # Set to False to disable bounding boxes in exported videos
...
class FaceTracker:
    ...
    def __init__(self, bbox, frame_num):
        ...
        self.history = [bbox]  # Track position history
...
class LitterEvent:
    ...
    def __init__(self, event_id, frame_num, obj, timestamp):
        ...
        self.has_culprit = False  # True if face or plate found
...
class LitterDetector:
    ...
    def __init__(self):
        ...
        # NEW: Store detection data for each buffered frame
        ...
        self.total_events = 0
        self.events_without_culprit = 0  # Count events filtered for no face/plate
        ...
        self.last_frame_time = 0  # NEW: For proper frame timing
    ...
    def initialize_detectors(self):
        ...
        try:
            ...
            # Load Haar Cascade for Russian license plates (or create custom)
            # Note: You may need to train or download a specific plate cascade
            ...
            if self.plate_cascade.empty():
                ...
                # Fallback: we'll use contour-based detection
                self.plate_cascade = None
            else:
    ...
    def detect_plates(self, frame):
        ...
        # Method 2: Contour-based plate detection (fallback)
    ...
    def update_face_tracking(self, face_detections):
        ...
        for det in face_detections:
            ...
            for face in self.tracked_faces:
                ...
                if score > 0.3:  # Lower threshold for faces
                    if score > best_score:
    ...
    def check_hand_movement_to_litter(self, face, litter_obj):
        ...
        # Estimate hand position as below face
        face_x1, face_y1, face_x2, face_y2 = face.bbox
        face_cx = (face_x1 + face_x2) / 2
        face_cy = (face_y1 + face_y2) / 2
        
        # Assume hand starts near face
        estimated_hand_start = (face_cx, face_y2)  # Below face
    ...
    def find_associated_culprit(self, litter_obj):
        ...
        # Check for nearby faces with hand movement
        for face in self.tracked_faces:
        ...
        # Check for nearby plates with car movement
        for plate in self.tracked_plates:
    ...
    def stop_recording(self):
        ...
        for event in self.active_events:
            if not event.snippet_complete:
                # Check if event has culprit before finalizing
                if event.has_culprit:
                    self.finalize_event_snippet(event)
                else:
                    # Discard event without culprit
    ...
    def load_event_snippet(self, index):
        ...
        self.last_frame_time = time.time()  # NEW: Initialize frame timing
    ...
    def process_frame(self, frame):
        ...
        # NEW: Store detection data for this frame
        detection_data = {
        ...
        # NEW: Add object detection data to buffer
        for obj in self.objects:
            x1, y1, x2, y2 = obj.bbox
        ...
        # Write to active events
        for event in self.active_events[:]:
            if hasattr(event, 'writer') and event.writer is not None:
                # NEW: Write frame with or without bounding boxes
    ...
    def show_help(self):
        """Display help"""
        print("\n" + "="*70)
        print("ENHANCED LITTER DETECTION - FACE & PLATE RECOGNITION")
        print("="*70)
        print("\nFEATURES:")
        print("- Detects litter events")
        ...
        while self.running:
            if self.playback:
                if not self.play_paused:
                    # NEW: Proper frame timing for real-time playback
                    ...
                    if elapsed >= frame_duration:
                        ...
                        # Use last frame if not enough time has elapsed
            ...
            cv2.imshow("Enhanced Litter Detection", frame)
            
            # NEW: Shorter wait time for smoother playback
